...
function ControladorRecomendaciones() {
    ...
    this.inicializarPreguntas = function() {
        ...
        $(".paso-1 .boton-corta-duracion").click(function() { /* agregue yo */
            self.duracion = 100;
            self.cargarSegundaPregunta();
        });
    ...
    this.pedirRecomendacion = function() {
        ...
        if (this.duracion) /*agregue yo*/
            query_params.duracion = this.duracion;

        if (Object.keys(query_params).length !== 0) {
            var query = $.param(query_params);
            var ruta = /*"/peliculas/recomendacion?";*/ "/recomendacion?"; //cambie la ruta yo
        } else {
            var ruta = /*"/peliculas/recomendacion";*/ "/recomendacion"; //cambie la ruta yo
            var query = "";
        }
